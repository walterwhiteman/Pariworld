...
export default defineConfig({
  ...
  define: {
    // These will be replaced by string literals at build time.
    // They read from process.env, which Render exposes.
    // Ensure these match the EXACT names you set in Render's environment variables.
    '__firebase_config': JSON.stringify(process.env.__firebase_config),
    '__app_id': JSON.stringify(process.env.__app_id),
    '__initial_auth_token': JSON.stringify(process.env.__initial_auth_token || ''), // Fallback to empty string if not set
  },
  server: {
    host: true, // This allows Vite's dev server to be accessed externally
  }
});
```

**Explanation for `vite.config.js` changes:**

* **`define` property:** This tells Vite/Rollup to replace certain global identifiers with static values at build time.
* **`process.env.__firebase_config`:** During the *build process* on Render, `process.env` will contain the environment variables you've set in your Render dashboard. This line instructs Vite to take the value of the `__firebase_config` environment variable from Render and embed it directly into your frontend JavaScript bundle.
* **`JSON.stringify()`:** This is crucial! Environment variables are always strings. `JSON.stringify()` ensures that the string value (which is a JSON string of your Firebase config) is correctly embedded as a *JavaScript string literal* in the final bundle, allowing `JSON.parse()` in `ChatPage.tsx` to work correctly.
* **`__initial_auth_token || ''`**: Provides a safe fallback if this variable isn't explicitly set in Render.

---

### 2. Update `src/pages/chat.tsx` (Add More Diagnostic Logs)

We'll add even more diagnostic logs to `ChatPage.tsx` to see the raw `__firebase_config` string *before* parsing, and what `firebaseConfig` looks like *after* parsing. This will be invaluable in confirming if Vite is correctly injecting the variable.
...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    try {
      if (!app) {
        ...
        console.log('__firebase_config raw (before parse):', typeof __firebase_config, __firebase_config);
        console.log('__app_id raw:', typeof __app_id, __app_id);
        console.log('__initial_auth_token raw:', typeof __initial_auth_token, __initial_auth_token);
        ...
        // DIAGNOSTIC LOG: Log the config being used after parsing
        console.log('Firebase Config AFTER PARSE:', firebaseConfig);
        console.log('App ID AFTER PARSE:', appId);
        console.log('Initial Auth Token PRESENT AFTER PARSE:', initialAuthToken ? 'Yes' : 'No');

        // Check if projectId exists in the parsed config
        if (!firebaseConfig.projectId) {
            throw new Error('"projectId" not provided in Firebase config after parsing.');
        }
  ...
  }, [socket, addNotification, isAuthReady, db, userId, location]);
  ...
  useEffect(() => {
    ...
    const unsubscribeRoomJoined = socket.on('room-joined', (data: { roomId: string; participants: string[] }) => {
      ...
      const systemMessage: ChatMessage = {
        ...
        roomId: data.roomId,
        sender: 'System',
        content: `You (${roomState.username}) joined the chat`,
        messageType: 'system',
        timestamp: new Date()
      };
      setRoomState(prev => ({
